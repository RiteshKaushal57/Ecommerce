...
const Cart = () => {
    ...
    const { addToCart, removeFromCart, cart, deleteProduct} = UseProductContext()
        ...
        <div className="flex flex-col md:flex-row py-16 max-w-6xl w-full px-6 mx-auto">
            <div className='flex-1 max-w-4xl'>
                <h1 className="text-3xl font-medium mb-6">
                    Shopping Cart <span className="text-sm text-indigo-500">{cart.length} {cart.length === 0 ? 'item': 'items'}</span>
                </h1>
                ...
                {cart.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button onClick={() => deleteProduct(product)} className="cursor-pointer mx-auto">
